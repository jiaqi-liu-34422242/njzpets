Create a single horizontal sprite strip for the Codex app digital pet `minji` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 2D chibi plush-doll desk pet, face-first resemblance to Minji, big glossy eyes, black hair with center part/top tuft, blue dress, white legs, black shoes.
