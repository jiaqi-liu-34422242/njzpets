Create a single horizontal sprite strip for the Codex app digital pet `minji` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 2D chibi plush-doll desk pet, face-first resemblance to Minji, big glossy eyes, black hair with center part/top tuft, blue dress, white legs, black shoes.
...
Repair attempt 1:
- The previous `running-right` strip failed QA: running-right used extraction method slots; regenerate the row or inspect slot slicing
