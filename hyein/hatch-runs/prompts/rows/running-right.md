Create a single horizontal sprite strip for the Codex app digital pet `hyein` in the state `running-right`.
...
Animation action: rightward flying locomotion loop inspired by the hovering flight style language of classic super-cute cartoon superheroes.


State-specific requirements:
- CRITICAL: this should read as airborne flight, not foot running.
- Match the visual language of a cute cartoon hovering-flight cycle: body lifted off the ground, forward travel implied by a streamlined pose, tiny arm/hand pose changes, subtle bobbing, and a compact flying silhouette.
- Keep both feet off the ground or tucked backward/under the body so the action reads as flying travel rather than stepping on a floor.
- The pet may lean slightly toward the travel direction, but must remain cute, simple, and readable at sprite scale.
- Show locomotion through airborne body pose, arm pose, leg tuck, hair/accessory sway, and small silhouette changes only.
- Do not draw speed lines, dust clouds, floor shadows, motion trails, detached motion effects, or any ground-contact cues.
