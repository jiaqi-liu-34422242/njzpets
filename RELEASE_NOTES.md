...
- `⇄ 切换下一只桌宠`：不用托盘 / 菜单栏，直接切换当前桌宠。
- `👥 多宠物并存`：不用托盘 / 菜单栏，直接显示全部成员桌宠。
- `🐾 Close This Pet`：单独关闭当前这只桌宠。
- `🔍 Scale Pet`：按档位循环缩放桌宠大小（也支持鼠标滚轮缩放）。
- `✏️ Edit Bubble Text`：打开气泡台词编辑窗口。
- `🗨️ 显示/隐藏气泡`：气泡默认不显示，需要时点击此按钮显示，再点一次隐藏。
...
- 多宠物同时显示可以直接点击桌宠操作面板里的 `👥 多宠物并存`，也可以从系统托盘 / 菜单栏图标的右键菜单里开启。
- 隐藏应用图标仍然在系统托盘 / 菜单栏图标的右键菜单里开启。
- 切换当前桌宠：
  - 右键托盘或菜单栏图标。
  - 点击 `切换下一个桌宠` 可直接循环切换。
  - 也可以打开 `切换桌宠`，选择指定成员。
